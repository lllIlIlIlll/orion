...
锚 = `./.active_project.<宿主pid>`，只对当前 TAU 进程有效：TAU 关闭即自动失活；多开 TAU 各自激活不同项目，互不干扰。（下文路径一律以 cwd 为基准，cwd 即 TAU 的 temp 目录；禁写 `temp/xxx` 前缀，会嵌套出 temp/temp）
...
2. 写文件锚，必须用 code_run（锚文件名含宿主 pid，ppid 即 TAU 宿主进程）：
...
TAU 关闭不需任何操作：锚随进程消亡自动失效，残留文件由插件下次启动时清扫
