...
<img src="assets/images/bar.jpg" width="880" alt="TAU Banner"/>
...
**TAU** is a minimal, self-evolving autonomous agent framework. Its core is just **~3K lines of code**. Through **9 atomic tools + a ~100-line Agent Loop**, it grants any LLM system-level control over a local computer — covering browser, terminal, filesystem, keyboard/mouse input, screen vision, and mobile devices (ADB).

> Design philosophy — **don't preload skills, evolve them.**

Every time TAU solves a new task, it automatically crystallizes the execution path into a reusable **Skill**. The longer you use it, the more skills accumulate — forming a personal skill tree grown entirely from 3K lines of seed code.

> 🤖 **Self-Bootstrap Proof** — Everything in this repository, from installing Git and running `git init` to every commit message, was completed autonomously by TAU. The author never opened a terminal once.
...
<table>
  ...
  <tr>
    <td><sub>While configuring a Discord bot, an hCaptcha <i>"Are you human?"</i> challenge pops up mid-task — TAU's real browser session passes it and the task continues. See <a href="#browser-realness-of-tau-web-tools">Browser Realness</a>.</sub></td>
...
> ⚠️ **Python version**: use **Python 3.11 or 3.12**. **Do not** use Python 3.14 — it is incompatible with `pywebview` and a few other TAU dependencies.
>
...
```bash
git clone https://github.com/lllIlIlIlll/orion.git TAU && cd TAU
...
```bash
python apps/tui/tui_v3.py   # Terminal UI (recommended)
...
Installing the package also installs a `tau` console script (equivalently, `python -m tau_cli`) that dispatches to every frontend and helper — no need to remember each file path:

| Command | Launches |
| :--- | :--- |
| `tau cli` | Interactive CLI chat (`agentmain.py`) — lightest option |
| `tau tui` / `tau tui2` | Terminal UI v1 / v2 |
| `tau launch` | Streamlit UI in a native webview shell |
| `tau gui` | PyQt desktop GUI |
| `tau hub` | Hub manager — system tray + browser panel |
| `tau configure` | First-time API-key / model wizard |
| `tau list` | Print every registered command |
| `tau status` / `tau update` | Inspect running processes / `git pull` + reinstall |

Run `tau` or `tau help` to see everything. For the recommended TUI, use `python apps/tui/tui_v3.py` directly.
...
3. If it still looks broken, ask TAU itself to fix it:
   > *"My experience using `apps/tui/tui_v3.py` in PowerShell / cmd / Git Bash on Windows is very poor — lots of incompatibility. Please refer to Claude Code's best practices for the Windows terminal and fix all font and rendering incompatibilities."*
...
TAU also supports IM apps such as Telegram, Discord, and Lark.

| Platform | Command |
| :--- | :--- |
| Telegram | `python apps/bots/tgapp.py` |
| Discord | `python apps/bots/dcapp.py` |
| Lark / Feishu | `python apps/bots/fsapp.py` |
...
In TAU, advanced capabilities are unlocked by **instructing the agent**, not by reading
docs or installing extras. Each instruction below makes TAU read its pre-installed SOPs
...
| Capability | Just tell TAU |
| :--- | :--- |
| 🌐 Web automation | *"Set up your web automation capability."* — TAU guides you through the one manual step: dragging the bundled Chrome extension into `chrome://extensions`. |
| 🔤 OCR | *"Set up your OCR capability with rapidocr and save it to memory."* |
| 👁️ Vision | *"Set up your vision capability from the template in memory/."* — TAU copies the template, wires it to your existing LLM keys, and self-tests. |
| 🖱️ Computer use | *"Probe this system and set up your computer-use capability."* |

> 💡 **About language**: the pre-installed SOPs are written in Chinese — TAU reads them
...
> itself a TAU task — on macOS/Linux, TAU swaps in the platform equivalents (window
...
TAU accomplishes complex tasks through **Layered Memory × Minimal Toolset × Autonomous Execution Loop**, continuously accumulating experience during execution.
...
> *TAU provides only **9 atomic tools**, forming the foundational capabilities for interacting with the outside world.*
...
Via `code_run`, TAU can dynamically install Python packages, write new scripts, call external APIs, or control hardware at runtime — crystallizing temporary abilities into permanent tools.

<div align="center">
  <img src="assets/images/workflow.jpg" alt="TAU Workflow" width="420"/>
  <br/><em>TAU Workflow Diagram</em>
</div>
...
This is what fundamentally distinguishes TAU from every other agent framework.
...
| Feature | **TAU** | OpenClaw | Claude Code |
| :--- | :---: | :---: | :---: |
...
| :---: | :--- | :--- | :--- |
| 1 | **Task Completion & Token Efficiency** | Can TAU complete hard tasks more cheaply than leading agents? | SOP-Bench, Lifelong AgentBench, RealFin-Benchmark |
...
<table>
  <tr>
    <td align="center" width="50%">
      ...
      <sub><b>Tool-use efficiency radar.</b> TAU dominates token, request, and tool-call axes while preserving quality across four task dimensions.</sub>
    </td>
    <td align="center" width="50%">
      <img src="assets/images/result_convergence.png" width="100%" alt="Cross-task self-evolution convergence"/><br/>
      <sub><b>Cross-task self-evolution.</b> Second- and third-run TAU executions converge to a stable low-cost regime across eight web tasks, while OpenClaw shows no such convergence.</sub>
    </td>
  </tr>
</table>

### Browser Realness of TAU Web Tools (TMWebdriver)

TAU web tools are powered by **TMWebdriver** — a local WebSocket server plus a Chrome extension — running through a **real, persistent Chrome/Chromium session** rather than a disposable headless sandbox, preserving cookies, login state, extensions, GPU/WebGL behavior, and normal browser-session fingerprints.

| Detection Service / Signal | Vanilla Headless Automation | TAU Web Tools | Notes |
| :--- | :---: | :---: | :--- |
...
- **2026-05-23** — 🆕 **TUI v3 released** (`apps/tui/tui_v3.py`). Block-based scrollback with proper resize reflow, per-terminal color profile for cross-terminal parity, and feature parity with v2.
...
- **2026-05-15** — 🖥️ **Desktop GUI released**. One-line installs ship a ready-to-run desktop app (`apps/TAU.exe`). Developers launch via `python launch.pyw`.
...
- **2026-05-12** — 🆕 **TUI v2 released** (`apps/tui/tuiapp_v2.py`). Refined Textual frontend with image-paste folding, file paste, block-delete, Ctrl+C copy, history navigation, and `/llm` / `/export` / `/continue` pickers.
...
- **2026-04-21** — 📄 [**Technical Report on arXiv**](https://arxiv.org/abs/2604.17091) — *TAU: A Token-Efficient Self-Evolving LLM Agent via Contextual Information Density Maximization*.
...
- **2026-03-08** — [Released "Dintal Claw" — a TAU-powered government-affairs bot](https://mp.weixin.qq.com/s/eiEhwo-j6S-WpLxgBnNxBg) *(Chinese)*.
- **2026-03-01** — [Featured by Jiqizhixin (机器之心)](https://mp.weixin.qq.com/s/uVWpTTF5I1yzAENV_qm7yg) *(Chinese)*.
- **2026-01-16** — TAU **V1.0** public release.
...
- [wangjc683/galley](https://github.com/wangjc683/galley) — Out-of-the-box local agent workbench with a bundled TAU runtime (CPython 3.11 + deps), native GUI/CLI, multi-session + Project orchestration, local-first.
...
> *Disclaimer: The official TAU channels are this GitHub repository and https://gaagent.ai. DintalClaw is currently the only officially authorized commercial partner; any other third-party website, organization, or individual using the TAU name is not official unless explicitly listed here.*
...
**TAU** 是一个极简、可自我进化的自主 Agent 框架。核心仅 **~3K 行代码**，通过 **9 个原子工具 + ~100 行 Agent Loop**，赋予任意 LLM 对本地计算机的系统级控制能力，覆盖浏览器、终端、文件系统、键鼠输入、屏幕视觉及移动设备（ADB）。

> 设计哲学 —— **不预设技能，靠进化获得能力。**

每解决一个新任务，TAU 就将执行路径自动固化为 Skill，供后续直接调用。使用时间越长，沉淀的技能越多，形成一棵完全属于你、从 3K 行种子代码生长出来的专属技能树。

> 🤖 **自举实证** — 本仓库的一切，从安装 Git、`git init` 到每一条 commit message，均由 TAU 自主完成。作者全程未打开过一次终端。
...
```bash
curl -fsSL https://raw.githubusercontent.com/lllIlIlIlll/orion/refs/heads/main/docs/installation_zh.md
```
...
> 💡 TAU 更推荐由 **Agent 在使用中自举环境**，而不是预先手动装完整依赖。

📖 完整引导流程见 [`docs/GETTING_STARTED.md`](docs/GETTING_STARTED.md)
📖 新手图文版：[飞书文档](https://my.feishu.cn/wiki/CGrDw0T76iNFuskmwxdcWrpinPb)
📘 完整入门教程（Datawhale 出品）：[Hello TAU](https://datawhalechina.github.io/hello-generic-agent/) · [GitHub](https://github.com/datawhalechina/hello-generic-agent)
...
安装包时会一并安装 `tau` 命令（等价于 `python -m tau_cli`），统一分发到各前端与工具，无需记忆各文件路径：

| 命令 | 启动内容 |
| :--- | :--- |
| `tau cli` | 命令行交互对话（`agentmain.py`），最轻量 |
| `tau tui` / `tau tui2` | 终端 UI v1 / v2 |
| `tau launch` | 原生 webview 壳包装的 Streamlit 界面 |
| `tau gui` | PyQt 桌面 GUI |
| `tau hub` | Hub 管理器（系统托盘 + 浏览器面板） |
| `tau configure` | 首次 API Key / 模型配置向导 |
| `tau list` | 打印所有已注册命令 |
| `tau status` / `tau update` | 查看运行进程 / `git pull` 并重装 |

运行 `tau` 或 `tau help` 查看全部命令；推荐 TUI 直接用 `python apps/tui/tui_v3.py`。
...
3. 仍然显示异常时，可以让 TAU 自己修一遍，参考 Prompt：
...
TAU 支持 Telegram、Discord、微信、QQ、飞书 / Lark、企业微信、钉钉等 IM 前端。

| 平台 | 启动命令 |
| :--- | :--- |
| Telegram | `python apps/bots/tgapp.py` |
| Discord | `python apps/bots/dcapp.py` |
| 微信 | `python apps/bots/wechatapp.py` |
| QQ | `python apps/bots/qqapp.py` |
| 飞书 / Lark | `python apps/bots/fsapp.py` |
| 企业微信 | `python apps/bots/wecomapp.py` |
| 钉钉 | `python apps/bots/dingtalkapp.py` |
...
TAU 通过 **分层记忆 × 最小工具集 × 自主执行循环** 完成复杂任务，并在执行过程中持续积累经验。
...
> *TAU 仅提供 **9 个原子工具**，构成与外部世界交互的基础能力。*
...
通过 `code_run`，TAU 可在运行时动态安装 Python 包、编写新脚本、调用外部 API 或控制硬件，将临时能力固化为永久工具。

<div align="center">
  <img src="assets/images/workflow.jpg" alt="TAU 工作流程" width="420"/>
  <br/><em>TAU 工作流程图</em>
</div>
...
| # | 维度 | 核心问题 | 使用的基准 |
| :---: | :--- | :--- | :--- |
| 1 | **任务完成度与 Token 效率** | TAU 能否以更低成本完成高难度任务？ | SOP-Bench、Lifelong AgentBench、RealFin-Benchmark |
...
<table>
  <tr>
    <td align="center" width="50%">
      ...
      <sub><b>工具使用效率雷达图。</b>TAU 在 Token、请求数和工具调用轴上全面领先，同时在四个任务维度上保持质量。</sub>
    </td>
    <td align="center" width="50%">
      <img src="assets/images/result_convergence.png" width="100%" alt="跨任务自我进化收敛曲线"/><br/>
      <sub><b>跨任务自我进化。</b>TAU 的第二轮和第三轮执行在 8 个 Web 任务上收敛至稳定的低成本区间。</sub>
    </td>
  </tr>
</table>
...
TAU Web 工具运行在**真实、持久化的 Chrome/Chromium 会话**中，而不是一次性的 headless 沙箱，因此可以保留 Cookie、登录态、扩展、GPU/WebGL 行为以及正常浏览器会话指纹。

| 检测服务 / 信号 | 普通 Headless 自动化 | TAU Web 工具 | 说明 |
| :--- | :---: | :---: | :--- |
...
- **2026-05-23** — 🆕 **TUI v3 正式发布**（`apps/tui/tui_v3.py`）。基于块的滚屏回看 + 正确的 resize 重排，每终端独立配色保证跨终端一致，并与 v2 达成功能对齐。
...
- **2026-05-15** — 🖥️ **桌面 GUI 发布**。一键安装会自带可直接运行的桌面端（`apps/TAU.exe`），开发者也可用 `python launch.pyw` 启动。
- **2026-05-14** — 🆕 **Conductor 子 Agent 编排**。派发、监督、自动清理并行子 Agent；与 `/btw` 旁路子 Agent 互补，提供一等公民级的任务委派原语。
- **2026-05-12** — 🆕 **TUI v2 正式发布**（`apps/tui/tuiapp_v2.py`）。重做视觉风格的 Textual 前端，支持图片粘贴折叠、文件粘贴、块删除、Ctrl+C 复制、历史导航，以及 `/llm` / `/export` / `/continue` 选择器。
- **2026-05-08** — 🆕 **Goal 模式**（`reflect/goal_mode.py`）。时间预算驱动的自驱循环 —— "持续优化 X N 小时"，预算没到不准提前交付。
- **2026-04-21** — 📄 [**技术报告已发布至 arXiv**](https://arxiv.org/abs/2604.17091) — *TAU: A Token-Efficient Self-Evolving LLM Agent via Contextual Information Density Maximization*。
...
- **2026-03-08** — [发布以 TAU 为核心的"政务龙虾" Dintal Claw](https://mp.weixin.qq.com/s/eiEhwo-j6S-WpLxgBnNxBg)。
- **2026-03-01** — [被机器之心报道](https://mp.weixin.qq.com/s/uVWpTTF5I1yzAENV_qm7yg)。
- **2026-01-16** — TAU **V1.0** 公开版本发布。
...
- [chilishark27/ga-manager](https://github.com/chilishark27/ga-manager)
- [wangjc683/galley](https://github.com/wangjc683/galley) —— 开箱即用的本地 Agent 工作台，自带 TAU 内核（内置 CPython 3.11 + 运行依赖），GUI/CLI 双原生、多 session + Project 编排、本地优先。
- [FroStorM/A3Agent](https://github.com/FroStorM/A3Agent/tree/workbench)
...
> *声明：TAU 官方渠道为本 GitHub 仓库和 https://gaagent.ai。DintalClaw 是目前唯一官方授权的商业合作方；除非在此处明确列出，其他使用 TAU 名义的第三方网站、机构、组织或个人均非官方。*
...
<a href="https://star-history.com/#lllIlIlIlll/orion&Date">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=lllIlIlIlll/orion&type=Date&theme=dark" />
    <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=lllIlIlIlll/orion&type=Date" />
    <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=lllIlIlIlll/orion&type=Date" />
  </picture>
</a>
