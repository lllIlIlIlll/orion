...
This module has zero TUI imports — both apps can depend on it without
...
# Repo root = parent of apps/.  Avoid hard-coding; both TUIs live next to
...
def build_conductor_prompt(args_text: str = "") -> str:
    """`/conductor <task>` → run `apps/conductor/conductor.py` on the task.
    ...
    if args_text:
        return f"请调用 apps/conductor/conductor.py 执行：{args_text}"
    return (
        "请调用 apps/conductor/conductor.py，根据后续指令完成多 subagent 编排。"
...
def list_launchable_services() -> list[dict]:
    ...
    Sources (hub.pyw EXCLUDES = goal_mode.py / chatapp_common.py / tuiapp.py):
      ...
      • apps/{bots,tui,web,desktop,conductor,acp}/*app*.py (not excluded)
          → 'stapp' → `python -m streamlit run … --server.headless=true`
            others   → `python apps/<carrier>/<f>`

    Returns [{name, cmd, doc, kind}] where `name` is the hub-style path
    ('reflect/foo.py' / 'apps/<carrier>/bar.py') and doubles as the
    picker value.
    """
    out: list[dict] = []
    refl = _ROOT / "reflect"
    ...
    if fe.is_dir():
        ...
        # — the flat apps/*.py glob no longer finds anything since the
...
def _match_service(cmdline: list[str], svc: dict) -> bool:
    ...
    arg (`reflect/foo.py` for reflect tasks, `apps/bar.py` for apps),
    ...
    if not cmdline:
        return False
    rel = svc["name"]  # 'reflect/foo.py' | 'apps/bar.py'
