...
function renderChannelList(channels) {
  ...
  const rows = (channels || []).filter((ch) => (ch.id || '').startsWith('apps/'));
...
const EXTRA_SERVICE_IDS = new Set(['apps/conductor/conductor.py', 'reflect/scheduler.py']);
...
function bridgeOfflinePanelServices() {
  return [
    ...
    },
    {
      ...
      name: 'apps/conductor/conductor.py',
      status: 'offline',
      running: false,
      pid: null,
      memMb: null,
...
function statusDisplayName(s) {
  ...
  if (s.id === 'apps/conductor/conductor.py') return t('proc.conductor');
