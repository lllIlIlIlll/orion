...
HTTP API:
  ...
  POST   /services/start        body: {"id":"apps/bots/qqapp.py"}
  POST   /services/stop         body: {"id":"apps/bots/qqapp.py"}
  GET    /services/logs?id=apps/bots/qqapp.py&tail=200
  GET    /services/panel
  GET    /services/taukey
...
class AgentManager:
    ...
    def make_agent(self, sess: Session):
        ...
        try: import apps.shared.cost_tracker as cost_tracker; cost_tracker.install()
        except Exception: pass
        old_cwd = os.getcwd()
        try:
...
_SERVICE_KEYS: Dict[str, tuple] = {
    "apps/bots/qqapp.py": ("qq_app_id", "qq_app_secret"),
    "apps/bots/dcapp.py": ("discord_bot_token",),
    "apps/bots/dingtalkapp.py": ("dingtalk_client_id", "dingtalk_client_secret"),
    "apps/bots/fsapp.py": ("fs_app_id", "fs_app_secret"),
    "apps/bots/tgapp.py": ("tg_bot_token",),
    "apps/bots/wecomapp.py": ("wecom_bot_id", "wecom_secret"),
}
...
def discover_im_services(ga_root: Path) -> List[dict]:
    out: List[dict] = []
    # IM bots live in apps/bots/ since the carrier-subdir restructure;
...
def discover_extra_services(ga_root: Path) -> List[dict]:
    ...
    # Carrier-subdir restructure: apps/conductor.py → apps/conductor/conductor.py.
    conductor = ga_root / "apps" / "conductor" / "conductor.py"
    if conductor.is_file():
        out.append({
            "id": "apps/conductor/conductor.py",
            "cmd": [sys.executable, "apps/conductor/conductor.py", "--no-browser"],
        })
    return out
