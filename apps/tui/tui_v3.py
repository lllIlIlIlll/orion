...
Merged from apps/tui/ (cjk, clipboard, renderer, protocol, core/sb)
into a single file so the v3 frontend ships as one drop-in module.
Run: `python -m apps.tui.tui_v3` or `python apps/tui/tui_v3.py`.
"""
...
# Make `apps/` parent (project root) importable so `from agentmain import …`
# works whether this file is run as `python -m apps.tui.tui_v3` or directly
# via `python apps/tui/tui_v3.py`.
...
from apps.shared import at_complete, workspace_cmd   # @ 补全 + /workspace（与 v2 共用）
...
_TIPS = {
    'en': [
        ...
        "Tip: /conductor <task> hands the task to apps/conductor/conductor.py for multi-subagent orchestration.",
...
_I18N: dict[str, dict[str, str]] = {
    'en': {
        ...
        'cmd.conductor.desc':   'hand task to apps/conductor/conductor.py for multi-subagent orchestration',
        ...
        'err.no_llm':           'No LLM configured — check taukey.py',
        ...
        # /scheduler picker (multi-pick reflect tasks / apps)
        ...
        'scheduler.empty':        '(no startable services: both reflect/*.py and apps/*app*.py are empty)',
    ...
    'zh': {
        ...
        'cmd.conductor.desc':   '调用 apps/conductor/conductor.py 做多 subagent 编排',
        ...
        'scheduler.empty':        '（没有可启动的服务：reflect/*.py 与 apps/*app*.py 均为空）',
...
class AgentBridge:
    ...
    def __init__(self, llm_no: int = 0):
        ...
        try:
            from apps.shared import continue_cmd as _cc
...
def _cost_str(agent) -> str:
    ...
    try:
        from apps.shared import cost_tracker
...
class SB:
    ...
    def _plan_card(self, w: int) -> list[str]:
        ...
        try:
            from apps.shared import plan_state
        except Exception:
            return []
    ...
    def _set_session_name(self, ag, name: str) -> None:
        ...
        try:
            from apps.shared import session_names
    ...
    def _reset_session(self, ag) -> None:
        ...
        from apps.shared import continue_cmd
    ...
    def _cmd(self, raw: str) -> None:
        ...
        if name in ('q', 'quit', 'exit'):
            ...
            try:
                from apps.shared import cost_tracker as _ct
        ...
        #     self.commit([_t('err.multi_session', name=name)])
            ...
            w = _term()[0]
            try:
                from apps.shared import session_names as _sn
            except Exception:
                _sn = None
            options: list[str] = []
            ...
            from apps.shared import review_cmd
            dq = queue.Queue()
            ...
            # frontend ignorant of SOP details; see apps/slash_cmds.py.
            ...
            from apps.shared import export_cmd
    ...
    def _btw(self, entry: list) -> None:
        ...
        from apps.shared import btw_cmd
        question = entry[0]
        try:
...
# ────────────────────────────────────────────────────────────────────────────
# entry: equivalent of apps/tui/__init__.py + __main__.py
# ────────────────────────────────────────────────────────────────────────────
