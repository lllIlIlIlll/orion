...
functionality migrated from apps/tui/tuiapp.py plus new commands:
...
from apps.shared.keysym import fmt_key, fmt_keys  # noqa: E402
...
_TIPS = (
    ...
    "Tip: /conductor <任务> 直接交给 apps/conductor/conductor.py 做多 subagent 编排。",
...
#   纯逻辑（索引/模糊/token）抽到 apps/at_complete.py，与 tui_v3 共用；
#   自动预读那一版见 temp/plan_v2_at_mention/autoread_version.py。
from apps.shared.at_complete import get_index, fuzzy_rank, find_at_token, format_pick, candidates_for, absolutize_mentions
...
_TASK_DIR_GLOB = os.path.join(APPS_DIR, '..', '..', 'temp', '_tui_v2_*')
...
import apps.shared.chatapp_common as chatapp_common  # noqa: F401
from apps.shared.chatapp_common import format_restore
from apps.shared.btw_cmd import handle_frontend_command as btw_handle
from apps.shared.review_cmd import handle as review_handle
from apps.shared.continue_cmd import list_sessions as continue_list, extract_ui_messages as continue_extract
import apps.shared.workspace_cmd as workspace_cmd
from apps.shared.export_cmd import last_assistant_text, export_to_temp, wrap_for_clipboard
from apps.shared.worldline import (
...
def default_agent_factory() -> Any:
    ...
    from apps.shared.slash_cmds import COMMIT_SIGNATURE_PROMPT
    agent = Tau()
    agent.inc_out = True
...
COMMANDS = [
    ...
    ("/conductor", "[task]",           "调用 apps/conductor/conductor.py 多 subagent 编排"),
...
def _filter_choices(all_choices: list, query: str) -> list:
    ...
    try:
        ...
        try:
            import apps.shared.continue_cmd as _cc  # type: ignore
        except Exception:
            _cc = None
...
class TauTUI(App[None]):
    ...
    def __init__(self, agent_factory: Optional[AgentFactory] = None) -> None:
        ...
        self._handlers: dict = {
            ...
            # slash_cmds bundle — see apps/slash_cmds.py for the prompt
        ...
        try:
            import apps.shared.cost_tracker as cost_tracker; cost_tracker.install()
        except Exception:
            pass
        # Patch TAU for /review in case chatapp_common didn't wire it.
        try:
            from agentmain import Tau as _GA
            import apps.shared.review_cmd as review_cmd; review_cmd.install(_GA)
        except Exception:
            pass
        # Drop session_names entries pointing at rotated-away logs.
        try:
            import apps.shared.session_names as session_names; session_names.gc()
        except Exception:
            pass
    ...
    def add_session(self, name: Optional[str] = None) -> AgentSession:
        ...
        try:
            agent.task_dir = os.path.join(APPS_DIR, '..', '..', 'temp',
        ...
        try:
            temp_dir = os.path.normpath(os.path.join(APPS_DIR, '..', '..', 'temp'))
        ...
        try:
            from apps.shared.continue_cmd import acquire_birth_lock
    ...
    def _rw_log_history(self, sess: AgentSession):
        ...
        try:
            import apps.shared.continue_cmd as _cc
    ...
    def action_drop_session(self) -> None:
        ...
        if dropped is not None:
            ...
            try:
                from apps.shared.continue_cmd import release_current
    ...
    def _cmd_rename(self, args, raw):
        ...
        try:
            import apps.shared.session_names as session_names
    ...
    def _cmd_model(self, args, raw):
        import apps.shared.model_cmd as model_cmd
    ...
    def _rw_rewind_root(self):
        ...
        return os.path.join(os.path.normpath(os.path.join(APPS_DIR, '..', '..', 'temp')), '.ga_rewind')
    ...
    def _cmd_continue(self, args, raw):
        ...
        choices = []
        try:
            import apps.shared.session_names as _sn
        except Exception:
            _sn = None
    ...
    def _continue_restore_apply(self, path: str, copy: bool) -> str:
        ...
        def _finish():
            ...
            try:
                from apps.shared.continue_cmd import iter_write_captures
                from agent_loop import get_pretty_json
            ...
            import apps.shared.plan_state as plan_state
            from apps.shared.continue_cmd import find_plan_entry
    ...
    def _cmd_cost(self, args, raw):
        try:
            import apps.shared.cost_tracker as cost_tracker
        except Exception as e:
    ...
    def _cmd_slash_inject(self, args, raw):
        ...
        """
        from apps.shared import slash_cmds
        text = (raw or "").strip()
    ...
    def _cmd_scheduler(self, args, raw):
        ...
        if not services:
            self._system("📋 没有可启动的服务（reflect/*.py 与 apps/*app*.py 均为空）"); return
    ...
    def _run_shell(self, cmd: str) -> None:
        ...
        sess.messages.append(ChatMessage("system",
                                         ...
                                         kind="system"))
        import subprocess
        from apps.shared.slash_cmds import detect_user_shell
...
def _warn_mintty():
    ...
    if direct_mintty:
        print(
            ...
            "    - Windows Terminal Git Bash: wt -p \"Git Bash\" python apps/tui/tuiapp_v2.py\n"
            "    - Windows Terminal:          wt python apps/tui/tuiapp_v2.py\n"
            "    - CMD:                       python apps\\tuiapp_v2.py\n"
            "    - PowerShell:                python apps/tui/tuiapp_v2.py\n"
            "\n"
